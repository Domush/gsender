...
const createServer = (options, callback) => {
  ...
  const mountPoints = uniqWith(
    [...ensureArray(options.mountPoints), ...ensureArray(config.get('mountPoints'))],
    isEqual
  ).filter((mount) => {
  ...
  mountPoints.forEach((mount) => {
    if (ensureString(mount.target).match(/^(http|https):\/\//i)) {
      log.info(
        `Starting a proxy server to proxy all requests starting with ${chalk.yellow(
          mount.route
        )} to ${chalk.yellow(mount.target)}`
      );
      ...
      routes.push({
        ...
        server: (options) => {
          ...
          proxy.on('proxyReq', (proxyReq, req, res, options) => {
            ...
            log.debug(
              `proxy.on('proxyReq'): modifiedPath=${chalk.yellow(
                proxyReq.path
              )}, originalPath=${chalk.yellow(originalPath)}`
            );
          });
          ...
          app.all(routeWithoutTrailingSlash, (req, res, next) => {
            ...
            if (
              ...
              const redirectUrl =
                routeWithoutTrailingSlash + '/' + url.slice(routeWithoutTrailingSlash.length);
              log.debug(
      ...
      log.info(
        `Mounting a directory ${chalk.yellow(
          JSON.stringify(directory)
        )} to serve requests starting with ${chalk.yellow(mount.route)}`
      );
