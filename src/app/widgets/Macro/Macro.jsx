...
class Macro extends PureComponent {
  ...
  onDragEnd = ({ source, destination, draggableId }) => {
    ...
    filtered.splice(destination.index, 0, {
      ...macro,
  ...
  render() {
    ...
    const [column1, column2] = [
      computeColumn({ columnName: 'column1' }),
      computeColumn({ columnName: 'column2' })
    ];
    ...
    const Container = ({ columns, children }) => {
      ...
      return (
        <div style={{ display: 'grid', gridTemplateColumns, overflowY: 'auto' }}>{children}</div>
      );
    };
