...
class ProbeWidget extends PureComponent {
  ...
  getInitialState() {
    ...
    if (touchplateType === TOUCHPLATE_TYPE_AUTOZERO) {
      ...
      toolDiameter =
        availableTools[0][units === METRIC_UNITS ? 'metricDiameter' : 'imperialDiameter'];
    }
  ...
  generateSingleAxisCommands(axis, thickness, params) {
    let { wcs, isSafe, probeCommand, retractDistance, normalFeedrate, quickFeedrate, units } =
      params;
    const workspace = this.mapWCSToPValue(wcs);
    let probeDistance =
      units === METRIC_UNITS
        ? this.PROBE_DISTANCE_METRIC[axis]
        : this.PROBE_DISTANCE_IMPERIAL[axis];
    ...
    if (axis !== 'Z') {
      ...
      const touchplateThickness =
        units === METRIC_UNITS ? touchPlateHeight : mm2in(touchPlateHeight);
      code = code.concat([
        this.gcode('G0', {
    ...
    code = code.concat([this.gcode('G90')]);
    return code;
  }
  ...
  generateMultiAxisCommands(axes, xyThickness, zThickness, params) {
    ...
    let XYProbeDistance =
      units === METRIC_UNITS ? this.PROBE_DISTANCE_METRIC.X : this.PROBE_DISTANCE_IMPERIAL.X;
    let ZProbeDistance =
      units === METRIC_UNITS ? this.PROBE_DISTANCE_METRIC.Z : this.PROBE_DISTANCE_IMPERIAL.Z;
    ZProbeDistance *= -1;
  ...
  generateProbeCommands() {
    ...
    const { axes } = this.determineProbeOptions(
    ...
    if (axesCount === 1) {
      if (axes.z) {
        this.generateSingleAxisCommands('Z', zThickness, gCodeParams).map((line) =>
          code.push(line)
        );
      }
      if (axes.y) {
        this.generateSingleAxisCommands('Y', xyThickness, gCodeParams).map((line) =>
          code.push(line)
        );
      }
      if (axes.x) {
        this.generateSingleAxisCommands('X', xyThickness, gCodeParams).map((line) =>
          code.push(line)
        );
      }
    }
    ...
    if (axesCount > 1) {
      this.generateMultiAxisCommands(axes, xyThickness, zThickness, gCodeParams).map((line) =>
        code.push(line)
      );
    }
  ...
  render() {
    ...
    return (
      <Widget fullscreen={isFullscreen}>
        ...
        <Widget.Content
          className={classNames(styles['widget-content'], styles.heightOverride, {
            [styles.hidden]: minimized
          })}
          active={active}
        >
          {state.modal.name === MODAL_PREVIEW && <RunProbe state={state} actions={actions} />}
          <Probe state={state} actions={actions} probeActive={probeActive} />
        </Widget.Content>
      </Widget>
    );
  }
