...
class Probe extends PureComponent {
  ...
  render() {
    ...
    const { canClick, availableProbeCommands, selectedProbeCommand, touchplate } = state;
              ...
              <div className={styles.axisButtonsWrapper}>
                {availableProbeCommands.map((command, index) => (
                  <FunctionButton
                    ...
                    className={
                      index === selectedProbeCommand ? styles.axisButtonActive : styles.axisButton
                    }
                  >
                    {index === selectedProbeCommand && (
                      <div className={styles.axisButtonActiveIndicator} />
                    )}
                    {command.id.split(' ')[0]}
                  </FunctionButton>
                ))}
