...
const ProgressArea = ({ state }) => {
  ...
  const outputFormattedTime = (givenTime) => {
    ...
    const strElapsedSecond = `${
      elapsedSecond[elapsedSecond.length - 2] !== undefined
        ? elapsedSecond[elapsedSecond.length - 2]
        : ''
    ...
    if (elapsedMinute > 59) {
      ...
      return `${flooredHours < 10 ? `0${flooredHours}` : flooredHours}h ${
        minutes < 10 ? `0${roundedMinutes}` : roundedMinutes
      }m`;
    }

    return `${elapsedMinute < 10 ? `0${elapsedMinute}` : elapsedMinute}m ${
      formattedSeconds < 10 ? `0${formattedSeconds}` : formattedSeconds
    }s`;
  };

  // eslint-disable-next-line no-restricted-globals
  const percentageValue = isNaN(((received / total) * 100).toFixed(0))
    ? 0
