  ...
  render() {
    ...
    const canPause =
      ...
      (isReady &&
        ...
        includes([WORKFLOW_STATE_RUNNING], workflowState));
    const canStop =
      isReady && includes([WORKFLOW_STATE_RUNNING, WORKFLOW_STATE_PAUSED], workflowState);
    const activeHold = activeState === GRBL_ACTIVE_STATE_HOLD;
    const workflowPaused =
      runHasStarted && (workflowState === WORKFLOW_STATE_PAUSED || senderInHold || activeHold);
      ...
      <div className={styles.workflowControl}>
        ...
        <div className={styles.relativeWrapper}>
          {workflowState !== WORKFLOW_STATE_RUNNING && (
            <>
              ...
              >
                {i18n._('Load File')}{' '}
                <i className="fa fa-folder-open" style={{ writingMode: 'horizontal-tb' }} />
              </button>
              <RecentFileButton />
              <div
                role="button"
                className={
                  fileLoaded ? `${styles.closeFileButton}` : `${styles['workflow-button-disabled']}`
                }
        ...
        {!workflowPaused && (
          <div className={styles.splitContainer}>
            ...
            >
              {i18n._('Outline')}{' '}
              <i className="fas fa-vector-square" style={{ writingMode: 'horizontal-tb' }} />
            </button>
            <button
              type="button"
              className={
            ...
            >
              {i18n._('Test Run')}{' '}
              <i className="fa fa-tachometer-alt" style={{ writingMode: 'horizontal-tb' }} />
            </button>
          </div>
        )}
        {canRun && (
        ...
        {showModal && (
          <Modal
            onClose={() => {
              this.setState((prev) => ({
                startFromLine: { ...prev.startFromLine, showModal: false }
              }));
              actions.closeModal();
            }}
          >
                ...
                <div className={styles.left}>
                  <div className={styles.greyText}>
                    ...
                    <div style={{ textAlign: 'center', marginBottom: '2rem' }}>
                      <p>
                        Start from line will take into account all movements prior to this line.
                      </p>
                      <p>
        ...
        {canPause && (
          ...
          >
            {i18n._('Pause Job')}{' '}
            <i className="fa fa-pause" style={{ writingMode: 'vertical-lr' }} />
          </button>
        )}
