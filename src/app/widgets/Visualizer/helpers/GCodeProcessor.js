...
export class GCodeProcessor {
  ...
  coord(coords, axis, value = null) {
    let axisNum =
      typeof axis === 'number' ? axis : this.vmState.axisLabels.indexOf(axis.toLowerCase());
  ...
  syncStateToMachine(options = {}) {
    ...
    if (shouldInclude('axisLabels')) {
      vmState.axisLabels = objtools.deepCopy(
        this.options.axisLabels || controller.axisLabels || ['x', 'y', 'z']
      );
    }
    ...
    if (shouldInclude('pos')) {
      vmState.pos = objtools.deepCopy(
        (controller.getPos && controller.getPos()) || controller.pos || this.zerocoord()
      );
    }
    if (shouldInclude('activeCoordSys')) {
      vmState.activeCoordSys =
        typeof controller.activeCoordSys === 'number' ? controller.activeCoordSys : null;
    }
    ...
    if (shouldInclude('storedPositions')) {
      vmState.storedPositions = objtools.deepCopy(
        controller.storedPositions || [this.zerocoord(), this.zerocoord()]
      );
    }
    ...
    if (shouldInclude('feed')) {
      vmState.feed =
        controller.feed ||
        (Array.isArray(this.options.maxFeed) ? this.options.maxFeed[0] : this.options.maxFeed);
    }
    ...
    if (shouldInclude('tool')) {
      vmState.tool =
        controller.tool !== null && controller.tool !== undefined ? controller.tool : null;
    }
  }
  ...
  runGcodeLine(gline) {
    ...
    let isSimpleMotion =
      motionCode &&
      (motionCode === 'G0' || motionCode === 'G1') &&
      (gline.has(motionCode) ? 1 : 0) +
        (gline.has('F') ? 1 : 0) +
        (gline.has('N') ? 1 : 0) +
        hasCoords.length ===
        gline.words.length;
