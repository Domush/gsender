...
class VisualizerWidget extends PureComponent {
  ...
  actions = {
    ...
    handleRun: () => {
      ...
      console.assert(
        includes([WORKFLOW_STATE_IDLE, WORKFLOW_STATE_PAUSED], workflow.state) ||
  ...
  componentDidUpdate(prevProps, prevState) {
    ...
    if (
      this.state.objects.coordinateSystem.visible !== prevState.objects.coordinateSystem.visible
    ) {
      this.config.set(
  ...
  shuttleControlEvents = {
    ...
    VISUALIZER_VIEW: (_, { type }) => {
      const { to3DView, toTopView, toFrontView, toRightSideView, toLeftSideView } =
        this.actions.camera;
    ...
    TOGGLE_SHORTCUTS: () => {
      ...
      const keybindingsArr = shortcuts.map((shortcut) =>
        shortcut.title === 'Toggle Shortcuts' ? shortcut : { ...shortcut, isActive: allDisabled }
      );
