...
class NavbarConnection extends PureComponent {
  ...
  render() {
    ...
    return (
      <div
        className={styles.NavbarConnection}
      ...
      >
        <div className={`${styles.NavbarConnectionIcon} ${styles[iconState]}`}>
          <i
            className={`fa ${this.renderConnectionStatusIcon(connected, connecting, alertMessage)}`}
          />
        </div>
        <div>
        ...
        <div className={styles.NavbarConnectionDropdownList}>
          ...
          {!connected && ports.length === 0 && (
            <div className={styles.noDevicesWarning}>No Devices Found</div>
          )}
          {!connected &&
            !connecting &&
            ports.map((port) => (
          ...
          {!connected && !connecting && unrecognizedPorts.length > 0 && (
            <UnrecognizedDevices
              ports={unrecognizedPorts}
              onClick={actions.toggleShowUnrecognized}
            />
          )}
          {!connected &&
            !connecting &&
            showUnrecognized &&
            unrecognizedPorts.map((port) => (
              <PortListing
                {...port}
                key={port.port}
                baudrate={baudrate}
