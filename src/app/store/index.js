  ...
  if (semver.lt(cnc.version, '1.0.1')) {
    ...
    const updateCommands = (commands) => {
      return commands.map((command) => {
        ...
        const foundDefaultCommand = defaultCommandKeys.find(
          (defaultCommand) => defaultCommand.id === command.id
        );

        return foundDefaultCommand
          ? { ...command, title: foundDefaultCommand.title, payload: foundDefaultCommand.payload }
          : command;
      });
    };
    ...
    if (
      ![TOUCHPLATE_TYPE_STANDARD, TOUCHPLATE_TYPE_AUTOZERO, TOUCHPLATE_TYPE_ZERO].includes(
        currentTouchplateType
      )
    ) {
  ...
  if (semver.lt(cnc.version, '0.7.4')) {
    ...
    const updateCommands = (commands) => {
      return commands.map((command) => {
        ...
        const foundDefaultCommand = defaultCommands.find(
          (defaultCommand) => defaultCommand.id === command.id
        );
