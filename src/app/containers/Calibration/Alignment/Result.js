...
const Result = ({ triangle, jogValues, onBack, onClose }) => {
  ...
  const calculateAlpha = ({ a, b, c }) =>
    Math.acos((a ** 2 + b ** 2 - c ** 2) / (2 * a * b)) * (180 / Math.PI);

  const calculateBeta = ({ trueHypotenuse, userHypotenuse, alpha }) =>
    userHypotenuse <= trueHypotenuse ? 90 - alpha : alpha - 90;

  const calculateFM = ({ b, beta }) => Number((b * Math.sin(beta * (Math.PI / 180))).toFixed(2));

  const movementCalculation = ({ currentStep, movedDistance, actualDistance }) =>
  ...
  const checkStepsPerMM = () => {
    ...
    const { $100: currentXStep, $101: currentYStep } = settings;
    ...
    const calculatedXStep = Math.round(
      movementCalculation({
        currentStep: Number(currentYStep),
        movedDistance: y,
        actualDistance: a
      })
    );
    const calculatedYStep = Math.round(
      movementCalculation({
        currentStep: Number(currentXStep),
        movedDistance: x,
        actualDistance: b
      })
    );
    ...
    Confirm({
      ...
      content: (
        <>
          ...
          {x?.should && (
            <p>
              <strong>$100</strong> - X steps/mm will be updated to <strong>{x?.amount}</strong>
            </p>
          )}
          {y?.should && (
            <p>
              <strong>$101</strong> - Y steps/mm will be updated to <strong>{y?.amount}</strong>
            </p>
          )}
        </>
      ),
        ...
        <>
          <div className={styles.result}>
            ...
            <p className={styles.resultText}>
              There was an error in the calculation, please check the values you provided.
            </p>
          </div>

          <FunctionButton primary onClick={onBack}>
            Go Back
          </FunctionButton>
        </>
      );
      ...
      return (
        <>
          <div className={styles.result}>
            <Icon type="success" />
            <p className={styles.resultText}>
              Your machine is squared properly, no adjustments are needed!
            </p>
          </div>

          <FunctionButton primary onClick={onClose}>
            Exit Calibration Tool
          </FunctionButton>
        </>
      );
    }
      ...
      return (
        <>
          <div className={styles.result}>
            <Icon type="warning" />
            <p>
              Your machine is pretty squared, but if you still want to have it full squared, move
              either the right y-axis rail backwards <strong>{fm}mm</strong> or the left y-axis rail
              forward by <strong>{fm}mm</strong>
            </p>
          </div>
          ...
          {stepsUpdate ? (
            <>
              <p className={styles.resultText}>
                We also noticed your steps-per-mm can be updated for improved accuracy on your
                machine, you can click the button below to do so.
              </p>
              <div style={{ display: 'flex', gap: '1rem' }}>
                <FunctionButton primary onClick={handleUpdateClick}>
    ...
    return (
      <>
        <div className={styles.result}>
          <Icon type="error" />
          <p>
            Your machine is off by <strong>{fm}mm</strong>, to properly square it, move either the
            right y-axis rail backwards <strong>{fm}mm</strong> or the left y-axis rail forward by{' '}
            <strong>{fm}mm</strong>
          </p>
        </div>
        ...
        {stepsUpdate ? (
          <>
            <p className={styles.resultText}>
              We also noticed your steps-per-mm can be updated for improved accuray on your machine,
