...
const Alignment = ({ onClose }) => {
      ...
      for (const action of foundAction.shapeActions) {
        ...
        if (foundShapeType) {
          if (clearPrevious) {
            ...
            setShapes((prev) => {
              const updated = prev[shapeType].map((shape) =>
                shape.id === shapeID ? { ...shape, label, isActive, show } : shape
              );
              return {
                ...prev,
                [shapeType]: updated
  ...
  const onChange = ({ id, checked, axis, value }) => {
    ...
    const updatedActions = actions.map((action) =>
      action.id === id ? { ...action, checked, hasBeenChanged: true } : action
    );
        ...
        <>
          <div className={styles.alignmentContainer}>
            ...
            >
              ...
              {introComplete && (
                <Step actions={actions} onChange={onChange} currentAction={currentAction} />
              )}
              ...
              {introComplete ? (
                <>
                  ...
                  <p style={{ width: '100%', fontWeight: 'bold' }}>
                    {stepFinished ? 'Proceed to the Next Step' : actionData?.description}
                  </p>
                </>
              ) : (
                <>
                  ...
                  />
                  <p style={{ width: '100%', fontWeight: 'bold' }}>
                    Since many CNCs run on two independent rails in the Y-axis, misalignment between
                    these rails can cause your cutting to become skewed.
                  </p>
                </>
              )}
            </div>
