...
const Triangle = ({ children }) => <div className={styles.triangle}>{children}</div>;

const CirclePoint = ({ position, label, isActive }) => (
  <div
    className={classnames(
      styles.circlePoint,
      isActive ? styles.circlePointActive : '',
      styles[position]
    )}
  >
...
const Arrow = ({
  ...
  return (
    <>
      {Label && (
        <div className={classnames(styles[labelPosition])}>
          {typeof Label === 'string' ? (
            ...
            <Label triangle={triangle} onTriangleChange={onTriangleChange} />
          )}
        </div>
      )}
      <div
      ...
      >
        {hasTop && (
          <div className={classnames(styles.arrowHead, isActive ? styles.arrowHeadActive : '')} />
        )}
        <div className={classnames(styles.arrowBase)} />
        {hasBottom && (
          <div className={classnames(styles.arrowTail, isActive ? styles.arrowTailActive : '')} />
        )}
      </div>
    </>
  );
