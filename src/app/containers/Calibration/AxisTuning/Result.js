  ...
  const renderResult = () => {
    ...
    return (
      <>
        ...
        <div>
          You requested to move <b>{requestedDistance}mm</b> but actually moved{' '}
          <b>{actualDistance}mm</b>. Your <b>{eepromSetting}</b> is currently set to{' '}
          <b>{eepromValue}</b>
        </div>
        <div>
          <b>
            <i>
              {eepromValue} × ({requestedDistance} ÷ {actualDistance}) = {roundedResult}
            </i>
          </b>
        </div>
      </>
        ...
        <FunctionButton
          ...
          onClick={() => {
            Confirm({
              ...
              onConfirm: () =>
                setEepromSetting(getEEPROMSetting(options.currentAxis), result.toFixed(3))
            });
          }}
        >
