...
class Workspace extends PureComponent {
      ...
      const content =
        comment.length > 0 ? (
          <div>
            ...
            <p>
              Line contained following comment: <b>{commentString}</b>
            </p>
          </div>
        ) : (
  ...
  onDrop = (files) => {
    ...
    reader.onloadend = (event) => {
      ...
      log.debug(
        'FileReader:',
        _.pick(file, ['lastModified', 'lastModifiedDate', 'meta', 'name', 'size', 'type'])
      );
  ...
  render() {
    ...
    const { port, modal, isDraggingFile, isDraggingWidget, showPrimaryContainer, reverseWidgets } =
      this.state;
      ...
      <ScreenAwake>
        <div style={style} className={classNames(className, styles.workspace)}>
          ...
          {modal.name === MODAL_FEEDER_WAIT && (
            <FeederWait title={modal.params.title} onClose={this.action.closeModal} />
          )}
          ...
          >
            <div className={classNames(styles.workspaceTable)}>
              ...
              <ConfirmationDialog />
              <div
                className={classNames(styles.workspaceTableRow, {
