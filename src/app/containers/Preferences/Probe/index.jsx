...
const Probe = ({ active, state, actions }) => {
  ...
  const values = {
    ...
    normalFeedrate:
      units === 'mm' ? probeSettings.normalFeedrate.mm : probeSettings.normalFeedrate.in,
    retractionDistance:
      units === 'mm' ? probeSettings.retractionDistance.mm : probeSettings.retractionDistance.in
  };
    ...
    <SettingWrapper title="Probe" show={active}>
      <GeneralArea>
        <GeneralArea.Half>
          ...
          />
          <ProbeSettings probeActions={probeActions} state={state} values={values} type={type} />
        </GeneralArea.Half>
