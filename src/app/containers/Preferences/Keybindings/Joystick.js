...
const Joystick = () => {
  ...
  const handleShortcutsToggle = (toggle) => {
    const updatedShortcuts = currentProfile.shortcuts.map((shortcut) => ({
      ...shortcut,
      isActive: toggle
    }));
    ...
    const updatedProfiles = profiles.map((profile) =>
      currentProfile.id === profile.id
        ? { ...profile, shortcuts: updatedShortcuts }
        : currentProfile
    );
    ...
    <div className={styles.container}>
        ...
        <>
          ...
          <div style={{ display: 'flex', flexDirection: 'column' }}>
            <div style={{ display: 'flex', gap: '1rem' }}>
              ...
              <Button
                primary
                onClick={() => handleShortcutsToggle(false)}
      ...
      {showAddProfile && (
        <AddProfileModal
          ...
          onAdd={(newProfiles) => setProfiles(newProfiles)}
        />
      )}
    </div>
  );
