...
const ProfileShortcutModal = ({ profile, shortcut, onClose, onUpdateProfiles }) => {
  ...
  const handleAddShortcut = () => {
    ...
    const newKeysName =
      shortcutName || gamepadShortcut.map((shortcut) => shortcut.buttonIndex).join(', ');
    ...
    <Modal onClose={onClose} size="small" title="Joystick Profile Action">
      <div className={styles.profileActionWrapper}>
        <h5 style={{ marginTop: 0, textAlign: 'center' }}>
          Press Any Button on Your Gamepad/Joystick
        </h5>
        ...
        <div>
          ...
          >
            ...
            {showIndicator ? (
              <strong>Button press detected</strong>
            ) : (
          ...
          <div style={{ margin: '1.5rem 0', textAlign: 'center', fontSize: '1.3rem' }}>
            ...
            {inUseError && (
              <span style={{ color: '#dc2626' }}>
                This Shortcut is Already in Use, Please Use a Different One
              </span>
            )}
            {gamepadShortcut && !inUseError && (
              <span style={{ color: '#3e85c7' }}>Shortcut is Available</span>
            )}
          </div>
