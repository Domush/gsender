...
const AddProfileModal = ({ onClose, onAdd }) => {
  ...
  const Availability = ({ isAvailable }) => {
    ...
    return <div className={styles.profileAvailable}>{availability()}</div>;
  };

  return (
    <ToolModal onClose={onClose} size="small" title="Add Gamepad/Joystick Profile">
      <div
        style={{
          padding: '1rem',
          display: 'flex',
      ...
      >
        ...
        <Button primary disabled={!gamepadInfo} onClick={handleAddProfile}>
          Add New Profile
        </Button>
      </div>
    </ToolModal>
